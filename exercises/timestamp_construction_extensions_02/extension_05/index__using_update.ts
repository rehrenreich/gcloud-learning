...
type MessageDocument = DataModel.IMessage<Date | admin.firestore.Timestamp>;
type MessageDocumentPartial = DataModel.IMessagePartial<Date | admin.firestore.Timestamp>;
...
export const saveMessageWithDateFromDataModel = onRequest(httpsOptions,
  async (request, response) => {
    ...
    if (docAlreadyExists) {
      ...
      await docRefTyped.update(messageDoc);
    } else {
      messageDoc = DataModel.createMessage(docId, message, undefined, false);
